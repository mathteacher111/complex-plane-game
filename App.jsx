...
const OPS = ["+", "-", "×"];
// 1차 게임용 랜덤 연산 묶음입니다.
// 곱셈을 더 자주 체감하도록 ×를 2번 넣었습니다.
const RANDOM_OPS = ["+", "-", "×", "×"];
const LIMIT = 20;
// 좌표를 더 크게 보여주기 위해 기본 확대 배율을 키웠습니다.
...
function operate(z, w, op) {
  if (op === "+") return add(z, w);
  if (op === "-") return sub(z, w);
  return mul(z, w);
}

function distance(z, target = { re: 0, im: 0 }) {
  return Math.sqrt((z.re - target.re) ** 2 + (z.im - target.im) ** 2);
}
...
function GameBoard({ players, currentPlayerIndex, target, history, mode, pendingWrong, onPickPoint }) {
  ...
  const size = LIMIT * 2 * zoom;
  const center = size / 2;
  const toX = (x) => center + x * zoom;
  const toY = (y) => center - y * zoom;
  ...
  function handlePointerDown(event) {
    ...
    const re = Math.round((svgX - center) / zoom);
    const im = Math.round((center - svgY) / zoom);
    if (Number.isFinite(re) && Number.isFinite(im) && Math.abs(re) <= LIMIT && Math.abs(im) <= LIMIT) {
      onPickPoint({ re, im });
    }
  }
  ...
  for (let i = -LIMIT; i <= LIMIT; i++) {
    ...
    gridLines.push(
      <line key={`v-${i}`} x1={toX(i)} y1={0} x2={toX(i)} y2={size} strokeWidth={i === 0 ? 4 : strong ? 1.8 : 0.8} className={i === 0 ? "stroke-slate-900" : strong ? "stroke-slate-300" : "stroke-slate-100"} />
    );
    gridLines.push(
      <line key={`h-${i}`} x1={0} y1={toY(i)} x2={size} y2={toY(i)} strokeWidth={i === 0 ? 4 : strong ? 1.8 : 0.8} className={i === 0 ? "stroke-slate-900" : strong ? "stroke-slate-300" : "stroke-slate-100"} />
    );
  }
  ...
  for (let i = -20; i <= 20; i += 5) {
    if (i !== 0) {
      labels.push(<text key={`x-label-${i}`} x={toX(i) - 6} y={center + 18} className="pointer-events-none fill-slate-700 text-[16px] font-black">{i}</text>);
      labels.push(<text key={`y-label-${i}`} x={center + 7} y={toY(i) + 4} className="pointer-events-none fill-slate-700 text-[16px] font-black">{i}</text>);
    }
  }
    ...
    <div className="rounded-[2rem] border-4 border-slate-900 bg-white p-3 shadow-2xl">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2 px-2">
        <div className="flex gap-2">
          <Button className="rounded-2xl text-xl" variant="outline" onClick={() => setZoom((prev) => Math.max(12, prev - 2))}>－</Button>
          <Button className="rounded-2xl text-xl" variant="outline" onClick={() => setZoom((prev) => Math.min(32, prev + 2))}>＋</Button>
        </div>
        <div className="text-xl font-black">복소평면</div>
        <div className="rounded-full bg-slate-900 px-4 py-2 text-base font-black text-white">범위 -20 ~ 20 · 확대 {zoom}</div>
      </div>
      <div className="w-full overflow-auto rounded-3xl bg-slate-50 p-2">
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="mx-auto block touch-none select-none rounded-2xl" onPointerDown={handlePointerDown}>
          ...
          {history.slice().reverse().map((h, idx) => (
            <circle key={`hist-${idx}`} cx={toX(h.after.re)} cy={toY(h.after.im)} r={4} fill={h.playerColor || "#94a3b8"} opacity={0.32} className="pointer-events-none" />
          ))}
      ...
      <div className="mx-auto max-w-4xl space-y-5 rounded-[2rem] bg-white p-6 shadow-2xl">
        ...
        <div className="rounded-3xl border-2 p-5">
          ...
          <div className="grid grid-cols-5 gap-3">
            {[1, 2, 3, 4, 5].map((count) => (
              <Button key={count} className="h-14 rounded-2xl text-lg font-black" variant={playerCount === count ? "default" : "outline"} onClick={() => setCount(count)}>
                {count}명
              </Button>
            ))}
          </div>
        ...
        <div className="rounded-3xl border-2 p-5">
          ...
          <input
            ...
            onChange={(event) => setRoundsPerPlayer(Math.max(1, Number(event.target.value) || 1))}
            className="w-full rounded-3xl border-2 border-slate-300 px-5 py-4 text-3xl font-black outline-none focus:border-slate-900"
          />
          <p className="mt-2 text-sm font-bold text-slate-500">예: 3이라고 적으면 모든 참가자가 3번씩 진행한 뒤 승자를 발표합니다.</p>
        </div>
          ...
          {players.map((player, idx) => (
            <div key={player.id} className="rounded-3xl border-2 p-5" style={{ backgroundColor: player.color.light }}>
              <div className="mb-4 flex items-center gap-3 text-xl font-black">
                ...
                <input
                  value={player.name}
                  onChange={(event) => setPlayerName(idx, event.target.value)}
                  placeholder={`${idx + 1}번 이름`}
                  className="min-w-0 flex-1 rounded-2xl border-2 border-white bg-white/80 px-4 py-3 text-xl font-black outline-none focus:border-slate-900"
                />
              </div>
                ...
                {PLAYER_COLORS.map((color) => {
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-2xl border-4 p-3 text-sm font-black transition ${isSelectedByThisPlayer ? "border-slate-900 scale-105" : "border-white"} ${isTakenByOtherPlayer ? "cursor-not-allowed opacity-35" : ""}`}
        ...
        <Button className="h-16 w-full rounded-3xl text-2xl font-black" onClick={startGame}>게임 시작</Button>
      </div>
    </div>
  );
}

export default function ComplexNumberGamePrototype() {
  ...
  const [opBag, setOpBag] = useState([...RANDOM_OPS]);
  ...
  const expected = useMemo(() => {
    ...
    return operate(pendingTurn.before, pendingTurn.number, pendingTurn.op);
  }, [pendingTurn]);

  function setCount(count) {
    setPlayerCount(count);
    setPlayers((prev) => {
      const next = Array.from({ length: count }, (_, idx) => prev[idx] || { id: idx + 1, name: `${idx + 1}번`, color: PLAYER_COLORS[idx % PLAYER_COLORS.length], position: { re: 0, im: 0 } });
      return next.map((p, idx) => ({ ...p, id: idx + 1, name: `${idx + 1}번` }));
    });
  ...
  function setPlayerName(index, name) {
    setPlayers((prev) =>
      prev.map((p, idx) =>
        idx === index ? { ...p, name: name.trimStart() || `${idx + 1}번` } : p
      )
    );
  }

  function startGame() {
    setPlayers((prev) => prev.map((p) => ({ ...p, position: { re: 0, im: 0 } })));
    setHistory([]);
    setCandidate(null);
    setPendingTurn(null);
    ...
    setTurnAdvancing(false);
    setGameStarted(true);
    setMessage(`${players[0]?.name || "1번"} 차례입니다. 주사위를 굴리세요.`);
  }
  ...
  function makeRankings(finalPlayers, gameMode) {
    const scored = finalPlayers.map((player) => ({
      id: player.id,
      name: player.name,
      color: player.color,
  ...
  function calculateFinalResult(nextPhaseResults) {
    ...
    const rows = players.map((player) => {
      ...
      const target = targetRows.find((row) => row.id === player.id);
      const randomPoint = random?.point || 0;
      const targetPoint = target?.point || 0;
      return {
        id: player.id,
        name: player.name,
        color: player.color,
        randomPoint,
        targetPoint,
        totalPoint: randomPoint + targetPoint * 2,
      };
  ...
  function finishGame(finalPlayers) {
    ...
    const nextFinalResult = mode === "target" ? calculateFinalResult(nextPhaseResults) : null;
    ...
    if (mode === "target" && nextFinalResult?.winner) {
      setMessage(`2차 게임 종료! 최종 승자는 ${nextFinalResult.winner.name} 입니다.`);
    } else {
      setMessage(`1차 게임 종료! 1차 승자는 ${finalWinner.name} 입니다. 2차 게임으로 넘어가세요.`);
    }
  }
  ...
  function drawOperation() {
    // 1차 게임에서는 각 참가자의 "첫 번째 턴"에만 곱셈을 완전히 제외합니다.
    // 첫 턴일 때는 기존 opBag에 ×가 남아 있어도 무시하고 +, - 중에서만 뽑습니다.
    const isFirstTurnForCurrentPlayer = mode === "random" && (turnCounts[currentPlayerIndex] || 0) === 0;

    if (isFirstTurnForCurrentPlayer) {
      const firstTurnOps = ["+", "-"];
      return firstTurnOps[Math.floor(Math.random() * firstTurnOps.length)];
    }

    const bag = opBag.length > 0 ? opBag : [...RANDOM_OPS];
    const index = Math.floor(Math.random() * bag.length);
    const picked = bag[index];
    const nextBag = bag.filter((_, idx) => idx !== index);
    ...
    const interval = window.setInterval(() => {
      ...
      if (ticks >= 10) {
        ...
        const fixedTurn = {
          ...
          playerColor: currentPlayer.color.fill,
          before: { ...currentPlayer.position },
          number: next,
          op: fixedOp,
        };
        setCandidate(next);
  ...
  function recordAttempt(turn, picked, expectedPoint, accepted, reason) {
    ...
    const record = {
      ...
      after: accepted ? expectedPoint : turn.before,
  ...
  function handlePickPoint(picked) {
    ...
    if (!turn) {
      setMessage("먼저 주사위를 굴리세요.");
      return;
    }

    // 판정은 React 상태값을 다시 읽지 않고, 이번 턴에 고정된 값만 사용합니다.
    ...
    if (!inBounds(expectedPoint)) {
      ...
      playTone("fail");
      setMessage(`정답은 ${formatComplex(expectedPoint)} 이지만 게임판 범위 밖입니다. 이번 턴은 이동하지 않습니다. 다음 차례로 넘어갑니다.`);
    ...
    if (samePoint(picked, expectedPoint)) {
      ...
      playTone("success");
      setMessage(`정답! ${turn.playerName} 말이 ${formatComplex(expectedPoint)} 에 안착했습니다. 다음 차례로 넘어갑니다.`);
  ...
  function reset() {
    ...
    setLastFeedback(null);
    setOpBag([...RANDOM_OPS]);
    setMessage("새 게임을 시작합니다. 1번 차례입니다. 주사위를 굴리세요.");
  }

  function randomTarget() {
    setTarget({ re: rollDie() * 3, im: rollDie() * 3 });
  }

  const currentScore = mode === "target" ? distance(currentPlayer.position, target) : distance(currentPlayer.position);

  if (!gameStarted) {
    return <PlayerSetup playerCount={playerCount} players={players} roundsPerPlayer={roundsPerPlayer} setRoundsPerPlayer={setRoundsPerPlayer} setCount={setCount} setPlayerColor={setPlayerColor} setPlayerName={setPlayerName} startGame={startGame} />;
  }
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_420px]">
          <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-[2rem] bg-slate-900 p-4 text-white shadow-2xl">
              ...
              <div className="flex gap-2">
                <Button className="rounded-2xl" variant={mode === "random" ? "secondary" : "outline"} onClick={() => {
                  if (candidate || rolling || turnAdvancing) return;
                  setMode("random");
                }} disabled={!!candidate}>1차: 멀리 가기</Button>
                <Button className="rounded-2xl" variant={mode === "target" ? "secondary" : "outline"} onClick={() => {
                  if (candidate || rolling || turnAdvancing) return;
                  setMode("target");
                  setPlayers((prev) => prev.map((p) => ({ ...p, position: { re: 0, im: 0 } })));
                  setCurrentPlayerIndex(0);
                  ...
                  setGameOver(false);
                  setWinner(null);
                  setMessage(`2차 게임 시작! 모든 말을 원점으로 되돌렸습니다. ${players[0]?.name || "1번"} 차례입니다. 주사위를 굴리세요.`);
                }} disabled={!!candidate}>2차: 목표점</Button>
              </div>
            </div>
              ...
              <CardContent className="space-y-4 p-5">
                <div className="rounded-3xl p-4" style={{ backgroundColor: currentPlayer.color.light }}>
                  ...
                  <div className="flex items-center gap-3 text-4xl font-black">
                    <span className="inline-block h-9 w-9 rounded-full shadow" style={{ backgroundColor: currentPlayer.color.fill }} />
                    {currentPlayer.name}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-3xl bg-slate-100 p-4 text-center">
                    <div className="text-sm font-black text-slate-500">현재 위치</div>
                    <div className="text-3xl font-black">({currentPlayer.position.re}, {currentPlayer.position.im})</div>
                  </div>
                  <div className="rounded-3xl bg-slate-100 p-4 text-center">
                    <div className="text-sm font-black text-slate-500">이번 연산</div>
                    <div className="text-5xl font-black">{candidate ? currentOp : "-"}</div>
                  </div>
                </div>
                ...
                {gameOver && winner && (
                  <div className="rounded-3xl bg-yellow-100 p-5 text-center ring-4 ring-yellow-400">
                    ...
                    <div className="mt-1 text-sm font-bold text-yellow-900">
                      {mode === "random"
                        ? `원점에서 거리 ${winner.score.toFixed(2)}`
                        : `목표점까지 거리 ${winner.score.toFixed(2)}`}
                    </div>

                    {mode === "target" && finalResult?.winner && (
                      <details className="mt-4 rounded-2xl bg-white p-4 text-left">
                        <summary className="cursor-pointer text-center text-lg font-black text-slate-900">
                          최종 승리자 확인하기
                        </summary>

                        <div className="mt-4 rounded-2xl bg-slate-100 p-4 text-center">
                          <div className="text-sm font-black text-slate-500">최종 우승</div>
                          <div className="mt-1 text-4xl font-black" style={{ color: finalResult.winner.color.fill }}>
                          ...
                          {finalResult.rows.map((row) => (
                            <div key={row.id} className="flex items-center justify-between rounded-2xl bg-white p-3 font-black">
                              <div className="flex items-center gap-2">
                                <span className="h-5 w-5 rounded-full" style={{ backgroundColor: row.color.fill }} />
                                {row.name}
                              </div>
                              <div>
                                {row.randomPoint} + {row.targetPoint}×2 = {row.totalPoint}
                              </div>
                            </div>
                          ))}
                        </div>
                ...
                <div className="grid grid-cols-2 gap-3 rounded-3xl bg-slate-50 p-4">
                  <DiceBox label="실수부" value={displayDice[0]} rolling={rolling} />
                  <DiceBox label="허수부" value={displayDice[1]} rolling={rolling} />
                </div>

                <div className="rounded-3xl border-2 p-4 text-center">
                  <div className="text-sm font-black text-slate-500">나온 복소수</div>
                  <div className="text-4xl font-black">{candidate ? formatComplex(candidate) : "-"}</div>
                </div>
                ...
                {mode === "target" && (
                  <div className="rounded-3xl border-2 p-4">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {OPS.map((op) => (
                        <Button key={op} className="h-16 rounded-3xl text-3xl font-black" variant={selectedOp === op ? "default" : "outline"} onClick={() => setSelectedOp(op)} disabled={!!candidate}>{op}</Button>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button className="h-16 rounded-3xl text-xl font-black" onClick={roll} disabled={rolling || !!candidate || turnAdvancing || gameOver}>{turnAdvancing ? "차례 전환" : rolling ? "굴리는 중" : "주사위"}</Button>
                  <Button className="h-16 rounded-3xl text-xl font-black" variant="secondary" onClick={nextTurn} disabled={rolling || !!candidate || turnAdvancing || gameOver}>차례 넘기기</Button>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <Button className="rounded-2xl" variant="outline" onClick={reset}>새 판</Button>
                  <Button className="rounded-2xl" variant="outline" onClick={backToSetup}>설정</Button>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-[2rem] border-4 border-slate-900 shadow-xl">
              <CardContent className="space-y-4 p-5">
                <div className="flex items-center justify-between gap-2">
                  <h2 className="text-2xl font-black">참가자 현황</h2>
                  {mode === "target" && <Button variant="outline" className="rounded-2xl" onClick={randomTarget} disabled={!!candidate}>목표 변경</Button>}
                </div>

                {mode === "random" ? (
                  <div className="rounded-2xl bg-amber-50 p-3 text-sm font-bold text-amber-900">1차 게임: 원점에서 멀수록 유리합니다.</div>
                ) : (
                  <div className="rounded-2xl bg-rose-50 p-3 text-sm font-bold text-rose-900">2차 게임: 목표점 {formatComplex(target)} 에 가까울수록 유리합니다.</div>
                )}
                ...
                <div className="grid gap-2">
                  {players.map((player, idx) => {
                    ...
                    return (
                      <div key={player.id} className={`flex items-center justify-between rounded-2xl p-3 ${idx === currentPlayerIndex ? "ring-4 ring-slate-900" : ""}`} style={{ backgroundColor: player.color.light }}>
                        <div className="flex items-center gap-2 font-black">
                          <span className="h-6 w-6 rounded-full" style={{ backgroundColor: player.color.fill }} />
                          {player.name} · ({player.position.re}, {player.position.im}) · {turnCounts[idx] || 0}/{roundsPerPlayer}턴
                        </div>
                        <div className="text-xl font-black">{playerScore.toFixed(2)}</div>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="max-h-64 overflow-auto rounded-2xl border">
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 bg-slate-100">
                      <tr>
                        <th className="p-2 text-left">턴</th>
                        <th className="p-2 text-left">참가자</th>
                        <th className="p-2 text-left">계산</th>
                    ...
                    <tbody>
                      {history.length === 0 ? (
                        <tr><td className="p-3 text-slate-500" colSpan={5}>아직 기록이 없습니다.</td></tr>
                      ) : history.map((h) => (
                        <tr key={h.turn} className="border-t">
                          <td className="p-2">{h.turn}</td>
                          <td className="p-2"><span className="mr-1 inline-block h-3 w-3 rounded-full" style={{ backgroundColor: h.playerColor }} />{h.playerName}</td>
                          <td className="p-2">{formatComplex(h.before)} {h.op} ({formatComplex(h.number)})</td>
                          <td className="p-2">{formatComplex(h.picked)}</td>
                          <td className={`p-2 font-black ${h.accepted ? "text-green-700" : "text-red-700"}`}>{h.accepted ? "정답" : h.reason}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
